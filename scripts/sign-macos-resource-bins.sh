...
# 明确说明本步骤会对哪些资源签名
echo "==> [sign-macos-resource-bins] 将对 node、uv/uvx 做 codesign（供 macOS 公证）"
...
ELECTRON_RESOURCES="${PROJECT_ROOT}/crates/agent-electron-client/resources"
# node-runtime 为 V8/Node，需 JIT 权限，否则 macOS 上会 SIGTRAP
NODE_RUNTIME_ENTITLEMENTS="${ELECTRON_RESOURCES}/node.entitlements"

# 使用一致的 identity 对单个二进制签名：Developer ID + 时间戳 + 硬化运行时
sign_one() {
  local bin_path="$1"
  if [ ! -f "$bin_path" ]; then
    return 0
...
# 对 node-runtime 使用带 JIT entitlement 的签名（仅 macOS Mach-O）
...
UV_BIN_DIR="${ELECTRON_RESOURCES}/uv/bin"
...
# 注意：node 是 V8/Node.js 运行时，需要 JIT 权限
NODE_BIN_DIR="${ELECTRON_RESOURCES}/node/bin"
...
# 3. 签名 resources/lanproxy/bin 下的 lanproxy
LANPROXY_BIN_DIR="${ELECTRON_RESOURCES}/lanproxy/bin"
if [ -d "${LANPROXY_BIN_DIR}" ]; then
  echo "==> [sign-macos-resource-bins] 签名 lanproxy 资源..."
  for f in "${LANPROXY_BIN_DIR}"/*; do
    if [ -f "$f" ] && [ -x "$f" ]; then
      if file "$f" | grep -q "Mach-O"; then
        sign_one "$f"
      fi
  ...
  echo "==> [sign-macos-resource-bins] 未找到 ${LANPROXY_BIN_DIR}，跳过 lanproxy 签名"
fi

echo "==> [sign-macos-resource-bins] node / uv / lanproxy 资源签名完成"
