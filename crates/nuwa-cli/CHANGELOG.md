...
- `nuwa-cli up` — one command to detect a usable local engine, log in/register with Nuwax, and start `serve --tunnel`. It supports `--saved-key`, `-u/--username` with interactive password, `NUWACLAW_PASSWORD` for non-interactive username/password registration, explicit `--engine`, and automatic engine selection when omitted.
