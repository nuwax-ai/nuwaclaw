...
### Main Process Services (20 files)
...
| **GUI Agent** | | |
| GUI Agent Server | `gui/guiAgentServer.ts` | HTTP server for /gui/* API (screenshot, input) |
| Screenshot Service | `gui/screenshotService.ts` | Cross-platform screenshots (desktopCapturer) |
| Input Service | `gui/inputService.ts` | Keyboard/mouse automation (@nut-tree/nut-js) |
| Permission Service | `gui/permissionService.ts` | Platform permission detection |
| Security Manager | `gui/securityManager.ts` | Token auth, rate limiter, audit log |
| System Prompt | `gui/systemPrompt.ts` | Agent system prompt generation |
| Engine Hooks | `engines/engineHooks.ts` | Generic env/prompt extension registry |
...
| `GUIAgentSettings.tsx` | GUI Agent config & permissions |
...
GUI Agent 使 AI 引擎能够通过 bash curl 调用本地 HTTP 服务完成屏幕截图和键鼠操作。
...
GUI Agent HTTP Server (Electron Main Process, 127.0.0.1 only)
    ├── POST /gui/screenshot    → desktopCapturer
    ├── POST /gui/input         → @nut-tree/nut-js
...
```typescript
// 启动
await window.electronAPI.guiAgent.start({ port: 60010 });
// 停止
await window.electronAPI.guiAgent.stop();
// 状态
const status = await window.electronAPI.guiAgent.status();
```

或通过设置 UI（GUIAgentSettings.tsx）操作。

### 配置（持久化存储键 `gui_agent_config`）

| 字段 | 默认 | 说明 |
|------|------|------|
| `enabled` | `false` | 是否启用 |
| `port` | `60010` | HTTP 端口 |
| `screenshotScale` | `0.5` | 截图缩放 |
| `screenshotFormat` | `'jpeg'` | 格式 |
| `screenshotQuality` | `80` | JPEG 质量 |
| `rateLimit` | `10` | 速率限制 (ops/s) |

### 低侵入集成（Engine Hooks）

GUI Agent 通过 `engineHooks.ts` 提供的通用扩展点自注册，核心引擎代码无 GUI 模块直接依赖：

- **`registerEnvProvider()`** → 注入 `GUI_AGENT_PORT` + `GUI_AGENT_TOKEN` 到引擎 env
- **`registerPromptEnhancer()`** → 追加 GUI Agent system prompt
- **`app.on('will-quit')`** → 自清理，无需修改 `main.ts`

所有注册均在 `guiAgentHandlers.ts` 的 `registerGuiAgentHandlers()` 中完成。
...
| Windows | 不需要 | 不需要 |
| Linux | X11 自动；Wayland 受限 | xdotool |

### 安全

- Bearer Token（UUID v4，每次启动重新生成）
- 仅绑定 127.0.0.1
- 令牌桶速率限制
- 审计日志（环形缓冲 1000 条）

> 详细文档见 `docs/GUI-AGENT-IMPLEMENTATION.md`
...
App quit requested
    ...
    ├── Stop GUI Agent Server (self-registered via will-quit hook)
    └── Close database
```
