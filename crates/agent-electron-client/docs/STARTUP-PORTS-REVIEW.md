...
### 2.3 `scripts/check-startup-ports.js`
...
| **DB 路径** | 脚本用 `path.join(home, '.nuwax-agent', 'nuwax-agent.db')`，Windows 为 `%USERPROFILE%\\.nuwax-agent\\nuwax-agent.db`；调用 sqlite3 时路径可转为正斜杠以规避反斜杠转义，且 Windows 下传 `shell: true` 便于执行 sqlite3.cmd。 |
...
2. **脚本与 shared 同步**：修改 `shared/startupPorts.ts` 的默认值或键时，记得同步改 `scripts/check-startup-ports.js` 的 DEFAULTS/LABELS 和键名；必要时可考虑从 build 产物 require 解析逻辑以彻底单源（需解决 path alias 与运行环境）。
