...
> 通过预置 `~/.nuwaxbot/nuwaxbot.json` 或环境变量，跳过手动向导完成客户端初始化。
...
```
nuwaxbot.json (quickInit scope)  →  环境变量  →  无配置（走正常向导）
```
...
| `workspaceDir` | string | 否 | `~/.nuwaxbot/workspace` | 工作区目录 |
...
│  ┌────────────────────────────────────────────────────┐     │
│  │  readQuickInitConfig()                              │     │
│  │  1. 读取 ~/.nuwaxbot/nuwaxbot.json → quickInit     │     │
│  │  2. 读取 NUWAX_* 环境变量                           │     │
...
**每次启动都读取配置**：无论 setup 是否已完成，都优先读取 `nuwaxbot.json` / 环境变量。有配置 → 覆盖 DB；无配置 → 使用 DB 已有值。
...
| `APP_DATA_DIR_NAME` | `shared/constants.ts` | `.nuwaxbot` 目录名 |
