...
describe('rawMcpServersEqual', () => {
  ...
  describe('相同配置 → 不触发重建', () => {
    ...
    it('env 字段不同时仍认为相等（env 每次由 getAppEnv 重新注入）', () => {
      const a = { time: makeStdio('/uv', ['tool', 'run', 'mcp-time'], { env: { TZ: 'UTC' } }) };
      const b = { time: makeStdio('/uv', ['tool', 'run', 'mcp-time'], { env: { TZ: 'Asia/Shanghai' } }) };
      expect(eq(a, b)).toBe(true);
    });

    it('allowTools 顺序不同时认为相等（排序后比较）', () => {
      const a = { time: makeStdio('/uv', ['run', 'mcp-time'], { allowTools: ['get_time', 'convert_time'] }) };
      const b = { time: makeStdio('/uv', ['run', 'mcp-time'], { allowTools: ['convert_time', 'get_time'] }) };
      expect(eq(a, b)).toBe(true);
    });

    it('denyTools 顺序不同时认为相等（排序后比较）', () => {
      const a = { time: makeStdio('/uv', ['run', 'mcp-time'], { denyTools: ['b', 'a'] }) };
      const b = { time: makeStdio('/uv', ['run', 'mcp-time'], { denyTools: ['a', 'b'] }) };
      expect(eq(a, b)).toBe(true);
    });
  });
  ...
  describe('配置实际变化 → 应触发重建', () => {
    ...
    it('args 变化时返回 false', () => {
      const a = { time: makeStdio('/uv', ['tool', 'run', 'mcp-time', '--local-timezone=UTC']) };
      const b = { time: makeStdio('/uv', ['tool', 'run', 'mcp-time', '--local-timezone=Asia/Shanghai']) };
      expect(eq(a, b)).toBe(false);
    });
    ...
    it('allowTools 内容变化时返回 false', () => {
      const a = { time: makeStdio('/uv', ['run', 'mcp-time'], { allowTools: ['get_time'] }) };
      const b = { time: makeStdio('/uv', ['run', 'mcp-time'], { allowTools: ['convert_time'] }) };
      expect(eq(a, b)).toBe(false);
    });

    it('denyTools 内容变化时返回 false', () => {
      const a = { srv: makeStdio('/uv', ['run', 'srv'], { denyTools: ['op1'] }) };
      const b = { srv: makeStdio('/uv', ['run', 'srv'], { denyTools: ['op2'] }) };
      expect(eq(a, b)).toBe(false);
    });
  });
  ...
  describe('远程 server（url 类型）', () => {
    ...
    it('URL 变化时返回 false', () => {
      const a = { remote: { url: 'https://mcp.example.com/v1' } as McpServerEntry };
      const b = { remote: { url: 'https://mcp.example.com/v2' } as McpServerEntry };
      expect(eq(a, b)).toBe(false);
    });
  });
  ...
  describe('修复验证：proxy 包装格式与原始格式对比', () => {
    ...
    it('原始格式与 proxy 包装格式不等（确认旧 Bug 场景确实会触发重建）', () => {
      ...
      const proxyEntry = makeStdio('mcp-proxy', ['--config-file', '/tmp/nuwax-mcp-configs/mcp-config-abc.json']);
