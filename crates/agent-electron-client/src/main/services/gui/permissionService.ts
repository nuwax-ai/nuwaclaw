...
function checkLinuxPermissions(): GuiPermissionInfo {
  ...
  const screenCapture: GuiPermissionState =
    displayServer === 'x11' ? 'granted' :
    displayServer === 'wayland' ? 'denied' : 'unknown';

  const accessibility: GuiPermissionState =
    xdotoolAvailable ? 'granted' : 'denied';
 ...
 * 打开系统权限设置页面
 */
export async function openPermissionSettings(type: 'screenCapture' | 'accessibility'): Promise<boolean> {
  try {
