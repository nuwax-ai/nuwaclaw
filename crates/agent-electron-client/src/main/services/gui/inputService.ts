...
async function ensureNut(): Promise<boolean> {
  ...
  try {
    const nut = await (new Function('s', 'return import(s)'))('@nut-tree/nut-js') as any;
    nutMouse = nut.mouse;
...
function resolveButton(button: string | undefined): any {
  ...
  switch (button) {
    case 'right': return nutButton.RIGHT;
    case 'middle': return nutButton.MIDDLE;
    default: return nutButton.LEFT;
  }
}
 ...
 * 执行输入操作
 */
export async function executeInput(action: InputAction, delayMs = 50): Promise<InputResponse> {
  const t0 = Date.now();

  const available = await ensureNut();
  if (!available) {
    throw new Error('@nut-tree/nut-js is not available. Please install it: npm install @nut-tree/nut-js');
  }
