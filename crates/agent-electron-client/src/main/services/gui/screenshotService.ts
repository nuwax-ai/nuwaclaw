...
export async function takeScreenshot(
  ...
  if (!targetDisplay) {
    throw new Error(
      `Invalid displayIndex: ${displayIndex} (${displays.length} displays available)`,
    );
  }
  ...
  log.debug(
    `${TAG} Capturing: display=${displayIndex}, size=${screenWidth}x${screenHeight}, scale=${scale}, thumbnailSize=${thumbnailWidth}x${thumbnailHeight}`,
  );
  ...
  if (!sources || sources.length === 0) {
    throw new Error(
      "No screen sources available. Check screen capture permissions.",
    );
  }
  ...
  if (image.isEmpty()) {
    throw new Error(
      "Screenshot is empty. Screen capture permission may not be granted.",
    );
  }
  ...
  log.debug(
    `${TAG} Screenshot captured: ${scaledSize.width}x${scaledSize.height}, format=${format}, size=${Math.round(base64.length / 1024)}KB, elapsed=${elapsed}ms`,
  );
