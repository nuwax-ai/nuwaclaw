  ...
  switch (act.type) {
    ...
    case "keyboard_type":
      ...
      if (act.text.length > MAX_TEXT_LENGTH) {
        throw new Error(
          `keyboard_type text too long (max ${MAX_TEXT_LENGTH} chars)`,
        );
      }
      break;
    case "keyboard_press":
    ...
    case "keyboard_hotkey":
      if (
        !Array.isArray(act.keys) ||
        !act.keys.every((k: unknown) => typeof k === "string")
      ) {
...
export function validateScreenshotRequest(
  ...
  if (body.format !== undefined) {
    if (body.format !== "png" && body.format !== "jpeg")
  ...
  if (body.displayIndex !== undefined) {
    if (
      typeof body.displayIndex !== "number" ||
      !Number.isInteger(body.displayIndex) ||
      body.displayIndex < 0
    ) {
...
async function handleRequest(
  ...
  try {
    ...
    if (pathname === "/gui/input" && method === "POST") {
      ...
      const delayMs =
        rawDelay !== undefined
          ? Math.max(0, Math.min(MAX_DELAY_MS, rawDelay))
          : undefined;
