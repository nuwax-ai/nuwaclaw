...
describe("spawnNoWindow", () => {
  ...
  describe("resolveNpmPackageEntry", () => {
    it("should return null if package.json not found", () => {
      ...
      expect(mockExistsSync).toHaveBeenCalledWith(
        path.join("/nonexistent/package", "package.json"),
      );
    });
    ...
    it("should resolve entry from object bin field", () => {
      ...
      const result = resolveNpmPackageEntry(
        "/packages/test-package",
        "test-command",
      );

      expect(result).toBe(
        path.join("/packages/test-package", "dist", "index.js"),
      );
    });
  ...
  describe("resolveNpmBinShimSpawnTarget", () => {
    const shimPath = path.join("/proj", "node_modules", ".bin", "tsx");

    it("should resolve npm bin shim to package JS entry", () => {
      mockExistsSync.mockImplementation((p: string) => {
        if (p === shimPath) return true;
        if (pathEndsWith(p, "package.json")) return true;
        if (pathEndsWith(p, "dist/cli.mjs")) return true;
        return false;
      });
      mockReadFileSync.mockReturnValue(
        JSON.stringify({
      ...
      const result = resolveNpmBinShimSpawnTarget(shimPath);

      expect(result).toEqual({
        binPath: path.join("/proj", "node_modules", "tsx", "dist", "cli.mjs"),
        isNative: false,
      });
    });

    it("should fall back to .CMD on Windows when package entry is missing", () => {
      const originalPlatform = Object.getOwnPropertyDescriptor(
        process,
        "platform",
      );
      Object.defineProperty(process, "platform", { value: "win32" });
      const cmdPath = path.join("/proj", "node_modules", ".bin", "tsx.CMD");

      mockExistsSync.mockImplementation((p: string) => {
        if (p === shimPath || p === cmdPath) return true;
        if (pathEndsWith(p, "package.json")) return false;
        return false;
      });

      try {
        const result = resolveNpmBinShimSpawnTarget(shimPath);
        expect(result).toEqual({ binPath: cmdPath, isNative: true });
      } finally {
        Object.defineProperty(process, "platform", originalPlatform!);
      }
    });

    it("should return null for paths outside node_modules/.bin", () => {
      mockExistsSync.mockReturnValue(true);

      expect(resolveNpmBinShimSpawnTarget("/usr/bin/node")).toBeNull();
    });

    it("should not treat native binary package entry as JS spawn target", () => {
      const esbuildShim = path.join("/proj", "node_modules", ".bin", "esbuild");
      mockExistsSync.mockImplementation((p: string) => {
        if (p === esbuildShim) return true;
        if (pathEndsWith(p, "package.json")) return true;
        if (pathEndsWith(p, "bin", "esbuild")) return true;
        return false;
      });
      mockReadFileSync.mockReturnValue(
        JSON.stringify({
      ...
      expect(resolveNpmBinShimSpawnTarget(esbuildShim)).toBeNull();
    });
  });
  ...
  describe("spawnNpmPackage", () => {
    ...
    it("should spawn with resolved entry", () => {
      ...
      try {
        const result = spawnNpmPackage("/packages/test-package", [
          "--port",
          "8080",
        ]);
    ...
    it("should pass binName for resolution", () => {
      ...
      try {
        ...
        expect(callArgs[1][0]).toBe(
          path.join("/packages/test-package", "dist", "bin.js"),
        );
      } finally {
        Object.defineProperty(process, "platform", originalPlatform!);
      }
