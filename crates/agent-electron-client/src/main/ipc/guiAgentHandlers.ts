...
} from "../services/engines/engineHooks";
...
export function registerGuiAgentHandlers(): void {
  ...
  ipcMain.handle(
    "guiAgent:start",
    async (_, config?: Partial<GuiAgentConfig>) => {
      try {
        const saved = readSetting(
          STORAGE_KEYS.AGENT_CONFIG,
        ) as GuiAgentConfig | null;
        const mergedConfig = {
  ...
  ipcMain.handle(
    ...
    async (_, config: Partial<GuiAgentConfig>) => {
      try {
        ...
        writeSetting(STORAGE_KEYS.AGENT_CONFIG, merged);
  ...
  // Request permission
  ipcMain.handle(
    "guiAgent:requestPermission",
    async (_, type: "screenCapture" | "accessibility") => {
      try {
  ...
  ipcMain.handle(
    "guiAgent:openPermissionSettings",
    async (_, type: "screenCapture" | "accessibility") => {
      try {
