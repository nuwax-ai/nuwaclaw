...
import {
  startWindowsMcp,
  stopWindowsMcp,
} from "../services/packages/windowsMcp.js";
...
export function createServiceManager(ctx: ServiceManagerContext) {
  ...
  const restartAllServices = async (): Promise<{
    ...
    // 2.5. Windows 平台启动 windows-mcp（必须在 MCP Proxy 后、Agent 前，因为 MCP 配置需注入）
    if (isWindows()) {
      try {
        const winMcpResult = await startWindowsMcp();
        results.windowsMcp = winMcpResult;
        if (winMcpResult.success && winMcpResult.port) {
          log.info(
            `[ServiceManager] Windows-MCP started on port ${winMcpResult.port}`,
          );
        } else {
          log.warn(
            "[ServiceManager] Windows-MCP start failed (non-blocking):",
            winMcpResult.error,
          );
        }
      } catch (e) {
        ...
        log.warn(
          "[ServiceManager] Windows-MCP start exception (non-blocking):",
          e,
        );
      }
    }
  ...
  const stopAllServices = async (): Promise<{
    ...
    if (isWindows()) {
      try {
        const winMcpResult = await stopWindowsMcp();
        results.windowsMcp = winMcpResult;
        log.info("[ServiceManager] Windows-MCP stopped");
      } catch (e) {
        results.windowsMcp = { success: false, error: String(e) };
        log.warn("[ServiceManager] Windows-MCP stop exception:", e);
      }
    }
