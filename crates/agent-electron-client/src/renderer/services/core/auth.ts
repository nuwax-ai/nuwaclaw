...
async function getSavedKey(domain?: string, username?: string): Promise<string | null> {
  if (domain && username) {
...
async function setSavedKey(value: string, domain?: string, username?: string): Promise<void> {
  if (domain && username) {
    const key = `${AUTH_KEYS.SAVED_KEYS_PREFIX}${normalizeDomain(domain)}_${username}`;
...
export async function loginAndRegister(
  ...
  try {
    ...
    if (response.serverHost && response.serverPort) {
      ...
      console.warn('[Auth] API 未返回 serverHost/serverPort，lanproxy 配置未更新');
    }
