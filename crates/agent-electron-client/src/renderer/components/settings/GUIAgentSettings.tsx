...
import type { GuiAgentConfig, GuiAgentStatus, GuiPermissionInfo, GuiPermissionState } from '@shared/types/guiAgentTypes';
...
const permissionLabel: Record<GuiPermissionState, { text: string; color: string }> = {
...
function GUIAgentSettings({ isOpen = true }: GUIAgentSettingsProps) {
  const [config, setConfig] = useState<GuiAgentConfig>(DEFAULT_GUI_AGENT_CONFIG);
  const [status, setStatus] = useState<GuiAgentStatus>({ running: false });
  const [permissions, setPermissions] = useState<GuiPermissionInfo | null>(null);
  ...
  const handleRequestPermission = async (type: 'screenCapture' | 'accessibility') => {
    try {
  ...
  const handleOpenPermissionSettings = async (type: 'screenCapture' | 'accessibility') => {
    try {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {permissions && (
        <Card
          size="small"
          title={<><SafetyCertificateOutlined /> 平台权限</>}
        ...
        >
          <Space direction="vertical" style={{ width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {permissions.screenCapture !== 'granted' && permissions.screenCapture !== 'not_needed' && (
                <Space>
                  <Button size="small" onClick={() => handleRequestPermission('screenCapture')}>
                    请求授权
                  </Button>
                  <Button size="small" icon={<SettingOutlined />} onClick={() => handleOpenPermissionSettings('screenCapture')}>
                    打开设置
                  </Button>
                </Space>
              )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {permissions.accessibility !== 'granted' && permissions.accessibility !== 'not_needed' && (
                <Space>
                  <Button size="small" onClick={() => handleRequestPermission('accessibility')}>
                    请求授权
                  </Button>
                  <Button size="small" icon={<SettingOutlined />} onClick={() => handleOpenPermissionSettings('accessibility')}>
                    打开设置
                  </Button>
                </Space>
              )}
            ...
            {permissions.platform === 'linux' && (
              <div>
                <Text type="secondary">
                  ...
                  {permissions.displayServer === 'wayland' && ' (Wayland 下部分功能受限)'}
                </Text>
