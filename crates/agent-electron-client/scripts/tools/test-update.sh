...
if [ -z "$APP_PATH" ]; then
  ...
  echo ""
  echo "  macOS:"
  echo "    NUWAX_UPDATE_SERVER=http://localhost:$SERVE_PORT open \"$OLD_RELEASE_DIR/mac-arm64/NuwaxBot.app\""
  echo ""
  echo "  或直接运行二进制:"
  echo "    NUWAX_UPDATE_SERVER=http://localhost:$SERVE_PORT \"$OLD_RELEASE_DIR/mac-arm64/NuwaxBot.app/Contents/MacOS/NuwaxBot\""
  echo ""
else
  echo "  找到旧版应用: $APP_PATH"
  echo ""
  if [ "$(uname)" = "Darwin" ]; then
    BINARY_PATH="$APP_PATH/Contents/MacOS/NuwaxBot"
    echo "  启动命令:"
