 ...
 * 从 ~/.nuwax-agent/nuwax-agent.db（Windows: %USERPROFILE%\.nuwax-agent\）读取配置。
 * 端口占用检查：优先统一用 bash 执行 scripts/check-port.sh（Windows 需先 npm run prepare:git 集成 Git Bash），
...
/** 优先用 bash 执行 scripts/check-port.sh（与 main 进程一致），否则回退 Node 内联逻辑 */
