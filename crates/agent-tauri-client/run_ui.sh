...
📖 使用方法
   ...
   开发模式（推荐）:
     ./run_ui.sh           启动开发服务器，自动打开应用窗口
     ./run_ui.sh dev       同上

   其他命令:
     ./run_ui.sh install   安装依赖
     ./run_ui.sh build     构建生产版本
     ./run_ui.sh check     Rust 代码检查
     ./run_ui.sh clean     清理构建产物
     ./run_ui.sh help      显示帮助
...
main() {
    ...
    case "$CMD" in
        ...
        clean)
            ...
            ;;
        *)
            error "未知命令: $CMD (使用 ./run_ui.sh help 查看帮助)"
    esac
}
