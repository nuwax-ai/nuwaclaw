...
- `nuwaclaw login` / `logout` / `status` / `config` — headless Nuwax account login (`domain` + `savedKey` model), no UI required.
  - When no `--saved-key`/`-u` is given and nuwaclaw-cli has no prior login of its own, `login` detects logins already saved by the Electron NuwaClaw client (read-only, from its local SQLite settings file) and offers to reuse one — a single match asks for confirmation, multiple matches show a picker. Imports the credential *value* only: the resulting reg call still uses nuwaclaw-cli's own device id, so it registers as an independent device from the Electron client rather than sharing its session. `--domain` narrows detection to matching entries. Covered by `tests/electronImport.test.ts` and the Electron-import cases in `tests/login.test.ts`.
...
- `doctor`'s Nuwax-login fix hint now mentions the Electron-client auto-detect (see `login`'s import above) when `~/.nuwaclaw/nuwaclaw.db` exists, instead of always suggesting manual `--domain`/`--saved-key` — only checks the file's existence (no sqlite read/lazy-install), since `doctor` is meant to be a cheap, side-effect-free, frequently-re-run diagnostic. Covered by new cases in `tests/doctorChecks.test.ts`.
...
- The `codex` engine downloads the `nuwax-codex-acp` binary from GitHub Releases on first use and caches it under `~/.nuwaclaw/engines/`.
...
- Electron-client login detection reads `~/.nuwaclaw/nuwaclaw.db` directly (a plain SQLite `settings` key-value table) via a lazily-installed `better-sqlite3` (same on-first-use install pattern as the claude adapter) — not a default dependency, and opened read-only. Deliberately keeps nuwaclaw-cli's own device id separate from the Electron client's rather than merging identities, matching the existing port separation (60016/60015 vs. Electron's 60005-60009) that lets both run on the same machine at once.
