...
- `nuwaclaw account list` / `account switch <account>` — list saved CLI accounts and switch the current default account. Switching re-registers with the selected account and refuses to run while `serve` is active, because serve/file-server/lanproxy/backend registration must be restarted together.
...
- `nuwaclaw up` — one command to detect a usable local engine, log in/register with Nuwax, and start `serve --tunnel`. It supports `--saved-key`, `-u/--username` with interactive password, `NUWACLAW_PASSWORD` for non-interactive username/password registration, explicit `--engine`, and automatic engine selection when omitted.
- `nuwaclaw update [version]` — upgrades the npm/pnpm-installed CLI package, with `--check`, `--dry-run`, `--package-manager npm|pnpm`, and `--registry`. It does not touch CLI credentials or service state.
- `nuwaclaw serve` — local HTTP API (`/computer/chat`, SSE `/computer/progress/:id`, `/computer/agent/status|stop|session/cancel`, `/computer/notify-resolved`, `/health`) for scripting/remote integration; `--tunnel` starts `nuwax-file-server` and a preintegrated lanproxy binary after login.
...
- CLI child processes strip Electron/client runtime variables (`NUWAX_*` login/port values, `NUWACLAW_SERVE_*`, ACP binary overrides, npm lifecycle noise) so a terminal-launched CLI does not accidentally inherit desktop-client state.
- `NUWACLAW_PASSWORD` is stripped from engine, lanproxy, file-server, and daemon child environments after being used for non-interactive login/up registration.
- `login --help`, `up --help`, and `account switch --help` document default-account reuse, multi-account JSON storage, `NUWACLAW_PASSWORD`, and the service-restart requirement for account switching.
...
- `nuwax-file-server` now runs with `TMPDIR`/`TMP`/`TEMP` scoped per port under `~/.nuwaclaw-cli/tmp/file-server-<port>`, isolating its package-level PID/lock files from any Electron-client, standalone file-server, or other CLI tunnel instance using a different port.
...
- nuwaclaw-cli deliberately does not import Electron-client login data. Keeping CLI savedKey/device id/local state under `~/.nuwaclaw-cli/` and preferring ports 60016/60015 keeps it separated from the Electron client's `~/.nuwaclaw/` data and 60005-60009 port range.
