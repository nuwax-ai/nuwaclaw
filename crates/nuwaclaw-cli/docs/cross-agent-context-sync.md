...
> 本文完善 `nuwaclaw-cli` 当前 `chat --ref-session` 的设计边界。参考对象：
...
`nuwaclaw-cli` 的核心协议是 ACP。所有真实 Agent 会话生命周期都必须继续由 ACP 承载：
...
2. **跨引擎 reference 走稳定引用**：新会话拿到一个短 handle，模型需要时再调用 `nuwaclaw` CLI 读取摘要/消息/产物。
...
`nuwaclaw chat --ref-session <engine>:<sessionId>` 已经采用了正确方向：

- 不做跨引擎 `session/load`。
- 首轮只注入一条提醒。
- 让模型按需运行 `nuwaclaw context digest/read --ref ... --json`。

首版已补齐：

- `nuwaclaw context list/read/digest/handoff` 命令组。
...
```bash
nuwaclaw chat --ref-session claude:c6e84245...
```

内部解析后使用完整 `ContextRef`。未来可以序列化成 URI：

```text
nuwaclaw://context/local-session?engine=claude&sessionId=...&cwd=...
```
...
```bash
nuwaclaw context list [--engine claude|codex]
nuwaclaw context read --ref claude:<sessionId> [--limit N] --json
...
需要上下文时，先运行：
nuwaclaw context digest --ref claude:<id> --json
若仍不够，再运行：
nuwaclaw context read --ref claude:<id> --limit 40 --json
不要假设未读取的内容。
</system-reminder>
```
...
```bash
nuwaclaw context mailbox send --to codex:<sessionId> --from claude:<sessionId> --text ...
nuwaclaw context mailbox list --for codex:<sessionId> --json
nuwaclaw context task add --title ... --owner codex:<sessionId>
nuwaclaw context task list --json
```
...
interface UnifiedSessionMessage {
  sessionId: string;          // nuwaclaw serve 的父会话/HTTP session
