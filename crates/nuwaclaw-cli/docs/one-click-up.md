# nuwaclaw 一键安装、注册与启动设计

本文描述 `nuwaclaw up` 能力：让用户从干净环境或已安装环境中，用尽量少的步骤完成 CLI 获取、Nuwax 注册、可用引擎选择、`serve --tunnel` 启动。

> 状态：已实现为 CLI 命令。npm 发布前可通过本地构建产物、`npm link` 或 `pnpm pack` 调试，见 [`local-debugging.md`](./local-debugging.md)。

## 目标

- 支持用户本机尚未安装 `nuwaclaw` 的干净环境。
...
```bash
pnpm dlx nuwaclaw@latest up --domain https://agent.nuwax.com --saved-key <key>
```

账号密码首次注册：

```bash
npx -y nuwaclaw@latest up --domain https://agent.nuwax.com -u <username>
```
...
```bash
NUWACLAW_PASSWORD='<password>' npx -y nuwaclaw@latest up \
...
```mermaid
flowchart TD
  A["用户运行 npx/pnpm dlx/nuwaclaw up"] --> B["检查 Node 与包依赖"]
  ...
  L --> M["写入 ~/.nuwaclaw-cli/credentials.json"]
  M --> N["启动 serve --tunnel"]
...
nuwaclaw-cli 不使用 SQLite；所有 CLI 登录数据都保存在 `~/.nuwaclaw-cli/credentials.json`。该 JSON 同时保存一个“当前默认账号”和一个轻量账号映射，账号 key 由 `domain + username` 组成，例如：
...
- 写入 CLI 自己的 `~/.nuwaclaw-cli/credentials.json`。
...
```bash
nuwaclaw serve --tunnel --engine <selected-engine>
```

并沿用现有隔离策略：

- CLI 数据根目录：`~/.nuwaclaw-cli`
...
`npx -y nuwaclaw@latest up ...` 可以解决“用户没安装 nuwaclaw”的问题，但不能解决“用户没有任何可用 Agent 登录态”的问题。
...
- `--domain --saved-key`：注册成功并写入 `~/.nuwaclaw-cli/credentials.json`。
