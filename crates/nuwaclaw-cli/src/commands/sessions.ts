...
export async function sessionsCommand(
  ...
  console.log(
    pc.dim(
      `\n共 ${sessions.length} 个本地会话。用 \`nuwaclaw chat --resume\` 续接。`,
    ),
  );
}
...
export async function sessionsSummaryCommand(
  ...
  const engine =
    merged.engine === "claude" || merged.engine === "codex"
      ...
      : undefined;
  if (!engine) {
    console.error(pc.red("[nuwaclaw] --engine 必须是 claude 或 codex"));
    process.exitCode = 1;
    return;
  }
  ...
  if (!sessionId) {
    console.error(pc.red("[nuwaclaw] 缺少 --session-id"));
    process.exitCode = 1;
    return;
  }
  ...
  if (!match) {
    console.error(
      pc.red(
        `[nuwaclaw] 未在本地 ${engine} 会话历史中找到 sessionId "${sessionId}"。`,
      ),
    );
    process.exitCode = 1;
    return;
