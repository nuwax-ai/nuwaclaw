...
function requireDefaultAccount(): void {
  ...
  throw new Error(
    "未找到可用于启动的默认账号。请先运行 `nuwaclaw login --domain <host> --saved-key <key>`，或 `nuwaclaw up --domain <host> -u <username>` 成功注册一次。",
  );
}
  ...
  try {
    ...
    console.log(
      pc.green(
        options.now
          ? "nuwaclaw 后台服务已安装并启动。"
          : "nuwaclaw 后台服务已安装，将在下次用户登录时自动启动。",
      ),
    );
    printPlatformNote();
  } catch (err) {
    console.error(
      pc.red(`[nuwaclaw] 安装后台服务失败：${(err as Error).message}`),
    );
    process.exitCode = 1;
  }
}
...
export async function serviceStartCommand(): Promise<void> {
  try {
    requireDefaultAccount();
    startService();
    console.log(pc.green("nuwaclaw 后台服务已启动。"));
  } catch (err) {
    console.error(
      pc.red(`[nuwaclaw] 启动后台服务失败：${(err as Error).message}`),
    );
    process.exitCode = 1;
  }
}
...
export async function serviceStopCommand(): Promise<void> {
  try {
    stopService();
    console.log(pc.green("nuwaclaw 后台服务已停止。"));
  } catch (err) {
    console.error(
      pc.red(`[nuwaclaw] 停止后台服务失败：${(err as Error).message}`),
    );
    process.exitCode = 1;
  }
}
...
export async function serviceUninstallCommand(): Promise<void> {
  try {
    uninstallService();
    console.log(pc.green("nuwaclaw 后台服务已卸载。"));
  } catch (err) {
    console.error(
      pc.red(`[nuwaclaw] 卸载后台服务失败：${(err as Error).message}`),
    );
    process.exitCode = 1;
  }
}
...
export async function serviceStatusCommand(): Promise<void> {
  try {
    ...
    console.error(
      pc.red(`[nuwaclaw] 查看后台服务失败：${(err as Error).message}`),
    );
    process.exitCode = 1;
  }
}
