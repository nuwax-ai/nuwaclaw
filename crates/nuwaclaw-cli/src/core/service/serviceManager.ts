...
export const SERVICE_LABEL = "com.nuwaclaw.cli";
export const WINDOWS_TASK_NAME = "NuwaclawCLI";
...
export function resolveCliEntryPath(argv1 = process.argv[1]): string {
  if (!argv1) throw new Error("无法定位当前 nuwaclaw CLI 入口文件。");
...
export function buildSystemdUserService(
  ...
  const envLines = Object.entries(env)
    ...
    .join("\n");
  return `[Unit]
Description=Nuwaclaw CLI headless agent
