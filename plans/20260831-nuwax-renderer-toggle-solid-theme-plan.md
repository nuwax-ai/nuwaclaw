...
- **追加：灰白纯色双端一致**（子模块 3ae474787 + dist 371a4c740，2026-08-31 用户实测反馈
  「客户端已生效、浏览器单独打开没有」后确认）：灰白调色板原整体门控在 `isNuwaClaw()`
  （Electron 桥）之后。放开边界——纯主题面（`isNuwaClawThemeActive` 谓词、init 监听注册）
  双端，显式选「纯色」+浅色即灰白；宿主面（init 默认写入女娲蓝+纯色、壳推送 syncTheme、
  品牌蓝主色兜底、沉浸壳避让）保持桌面专属；浏览器默认态观感不变。测试 12/12
  （新增浏览器默认态不生效/不写默认 + 浏览器显式纯色灰白生效两用例）。
