...
| 函数 | 功能 |
|------|------|
| `checkPermission` | 调用 Rust 检查权限 |
| `checkAllPermissions` | 批量检查 |
| `requestPermission` | 请求权限 |
| `openSystemSettings` | 打开系统设置 |
| `startPermissionMonitor` | 启动监控 |
| `stopPermissionMonitor` | 停止监控 |
| `onPermissionChange` | 监听权限变化 |
| `PERMISSION_MAPPING` | 权限名称映射 |
| `SETTINGS_URLS` | 系统设置 URL 映射 |

#### 前端服务层 (`src/services/permissions.ts`)

| 功能 | 说明 |
|------|------|
| `PermissionsService` | 权限服务单例 |
| 缓存机制 | 5秒缓存过期 |
