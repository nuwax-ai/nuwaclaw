...
## 如何验证「开机自启动」已生效
- **托盘/界面**：切换后托盘菜单「开机自启动」旁的勾选会更新；若已接入系统通知，会弹出「已开启/已关闭开机自启动」提示。
- **日志**：终端或日志文件中会出现 `[Tray] 开机自启动已启用` 或 `已禁用`。
- **系统级确认**（可选）：**macOS** 系统设置 → 通用 → 登录项，或 `~/Library/LaunchAgents/` 下 plist；**Windows** 任务管理器 → 启动；**Linux** `~/.config/autostart/`。可通过 `autolaunch_diagnose` 获取配置路径。
- **真实生效**：重启电脑后确认应用是否在登录时自动启动。
