...
Admin Server 是 Nuwax Agent 客户端的独立管理接口服务，用于：
1. **服务重启**：重启本地服务（排除 Lanproxy）
2. **健康检查**：检查代理服务通道健康状态

Admin Server 与业务服务（Computer Server）分离，独立运行在指定端口。
...
Admin Server 端口可在客户端"服务配置"页面修改，配置会持久化到 `step1_config.adminServerPort` 字段。
...
```bash
curl http://127.0.0.1:60007/admin/health
```
...
```bash
curl -X POST http://127.0.0.1:60007/admin/services/restart
```
...
|------|------|
| `0000` | 全部服务启动成功 |
| `1001` | 部分服务启动失败，`message` 中包含失败详情，`data` 中列出各服务结果 |
...
{
  "code": "1001",
  "message": "部分服务启动失败: guiAgentServer: 启动失败",
  "data": {
    "mcpProxy": { "success": true },
...
```bash
curl http://127.0.0.1:60007/admin/health/lanproxy
```
...
| `adminServerPort` | number | 是 | Admin Server 端口，默认 60007 | **后端需新增此字段** |
...
| `adminServerPort` | number | 是 | Admin Server 端口，默认 60007 |
...
```
Electron 客户端
├── GUI Agent Server :guiMcpPort (默认 60008)
│   └── /mcp  (streamable-http)
├── MCP Proxy :18099
...
```
客户端 (127.0.0.1)
├── Admin Server :60007  (管理接口)
│   ├── GET  /admin/health
│   ├── POST /admin/services/restart
│   └── GET  /admin/health/lanproxy
├── Computer Server :60006  (Agent HTTP API)
│   └── /computer/*, /chat/*
├── File Server :60005  (文件服务)
...
1. **端口可配置**：Admin Server 默认 60007，可在客户端修改。如端口变更，调用方需使用新端口。

2. **Admin Server 常驻**：Admin Server 与业务服务分离，独立运行。重启业务服务时不影响 Admin Server 响应。
